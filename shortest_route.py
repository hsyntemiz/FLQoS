...
class Shortest_Route(app_manager.RyuApp):
    ...
    def install_flow_bidirect_version2(self, dp_first, dp_last, flow_info, path, path_version, routing_mode=None, policy=None):
        ...
        try:
            ...
            #self.logger.info("################### flow_path_map =>MY FLOW:%s ---  X TUPLE X:%s == ",myflow,self.flowPathMap[myflow])
  ...
  # Geri don buraya!!!!
        ...
        try:
            ...
            if datapath_first.id == datapath_last.id:
                ...
                if routing_mode == None:
                    ...
                    # possible_paths = self.sort_paths_by_bw_used(datapath_first.id, datapath_last.id,routing_mode)
                ...
                if routing_mode == 'bw':
                    ...
                    #
                    #
                    # self.logger.info("POLICY -BW- !!!! ESTABLISHED##### Path is: %s -Path of bw:%s - POLICY:%s ", path_,path_of_bw,policy)

                    (path_,path_of_bw) = self.find_path(datapath_first.id, datapath_last.id,routing_mode)
                    self.logger.info("POLICY # %s #!!!! YASEMINBW##### Path is: %s -Path of bw:%s - POLICY:%s ", routing_mode, path_,path_of_bw,policy)

                    # if path_of_bw < (policy[-1] * 1000):
                    #     self.logger.info("POLICY -BW- BW REQUIREMENT(%s bps) NOT MET!!! The system offers (%s) bps ", policy[-1]* 1000,
                    #                  path_of_bw)
                    # else:
                    #     self.logger.info("POLICY -BW- BW REQUIREMENT(%s bps) SATISFIED!! The system offers (%s) bps ", policy[-1]* 1000,
                    #                  path_of_bw)
                    #


                if routing_mode == 'delay':

                    # possible_paths = self.sort_paths_by_delay(datapath_first.id, datapath_last.id,routing_mode)
                    # self.logger.info("MODE_DELAY----PATHS[%s]==:", possible_paths)
                    # possible_paths = list(possible_paths)
                    # path_ = possible_paths[0][0]
                    # path_of_delay = possible_paths[0][1]

                    (path_, path_of_bw) = self.find_path(datapath_first.id, datapath_last.id, routing_mode)
                    self.logger.info("POLICY # %s #!!!! YASEMIN DELAY##### Path is: %s -Path of bw:%s - POLICY:%s ",
                                     routing_mode, path_, path_of_bw, policy)

                    # if path_of_delay > policy[-2]:
                    #     self.logger.info("DELAY REQUIREMENT(%s ms) NOT MET!!! The system offers (%s)ms ", policy[-2],
                    #                  path_of_delay)
                    # else:
                    #     self.logger.info("DELAY REQUIREMENT(%s ms) MET!!! The system offers (%s) ", policy[-2],
                    #              path_of_delay)

                    #self.logger.info("MODE_DELAAAY----PATHS[%s]==best-path[%s]:", possible_paths, path)

                if routing_mode == 'delayBACKUP':
                    possible_paths = self.sort_paths_by_delay(datapath_first.id, datapath_last.id,routing_mode)
                    self.logger.info("MODE_DELAY----PATHS[%s]==:", possible_paths)
                ...
                if routing_mode == 'combined':
                    self.logger.info("PATH FOR COMBINED METRIC: SEND QOS ICI ")
                    gul_paths_delay = self.sort_paths_by_delay_resv(datapath_first.id, datapath_last.id)

                    delay_path_list = []
                    for delay_path in gul_paths_delay:
                        if delay_path[0] < policy[-2]:
                            delay_path_list.append(delay_path[0])

                    if delay_path_list:

                        self.logger.info("PATH FOR COMBINED METRIC 2: SEND QOS ICI ")

                        bw_path_list = []
                        for path_ in delay_path_list:
                            path_tuple = path_, self.path_bw_calculate_usage(path_)
                            bw_path_list.append(path_tuple)


                        self.logger.info("PATH FOR COMBINED METRIC 3: SEND QOS ICI ")

                        bw_path_list.sort(key=lambda tup: tup[1], reverse=True)

                        self.logger.info("combineeeeeeeeeeeee --%s--",bw_path_list)


                        bw_path_list = list(bw_path_list)
                        path_ = bw_path_list[0][0]
                        path_of_bw = bw_path_list[0][1]

                        self.logger.info("PATH (%s)FOR COMBINED METRIC:  BW:%s --- Delay:%s--", path_, path_of_bw,
                                     self.path_delay_calculate(path_))

                        if int(path_of_bw) > int(policy[-1] * 1024):
                            path_=[]
                            self.logger.info("COMBINED!BW REQUIREMENT(%s kbps) NOT MET!!! The system offers (%s) ",
                                         policy[-1], path_of_bw)
    ...
    def dijkstra(self, graph, initial,mode=None):
        ...
        while nodes:
            ...
            for edge in graph[min_node]:
                self.logger.info('dijkstraaaaaaaaaaaaaaaaaa loopP edge: %s --- min_node_removed_: %s ', edge,min_node)
                ...
                if edge not in visited or weight[3] > visited[edge][3]:
                    if edge not in visited:
                        self.logger.info('dijkstraaaaaaaaaaaaaaaaaa4444 Edge is not visited: %s --', visited)
        ...
        self.logger.info('dijkstraaaaaaaaaaaaaaaaaa visited: %s of path:%s ', visited,path)
    ...
    def shortest_fuzzy_path(self, graph, origin, destination,mode=None):
        ...
        self.logger.info(' shortest_fuzzy_pa dijkstraaaaaaaaaaaaaaaaaa 3333 %s - %s',visited[destination],list(full_path))

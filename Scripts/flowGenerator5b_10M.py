...
links=[[1,2],[1,3],[2,3],[2,4 ],[3,4],[3,5],[4,5],   [2,6],[4,6],[6,9],[9,10],[7,10],[7,8],[4,7],[5,8],[8,12],[11,12],[7,12],[10,11]     ]
print links[0][0]
print links[0][1]
...
def background(start,src,dst,port,duration,delay):
	if 4000<port<5000:
		qos_type=1
	if 8000<=port<=9000:
		qos_type=0
	...
	if 4899<port<5000:
        ...
        if back:
                ...
                rec_report='./log/B%s_reciever_reportVOIP_%s_to_%s_port_%s_%s.log' % (start,src.IP(), dst.IP(),port,qos_type)
                send_report='./log/B%s_sender_reportVOIP_%s_to_%s_port_%s_%s.log' % (start,src.IP(), dst.IP(),port,qos_type)
	...
	cmd_src='../D-ITG/bin/ITGSend -T UDP -rp %s -a %s -c 1000 -C 720 -t %s -Sdp %s -x %s &' % (port,dst.IP(),duration*1000,signal_port,rec_report)
...
#durations=np.random.randint(low=MIN_DURATION,high=MAX_DURATION, size=LINK_COUNT)
#delays=np.random.randint(low=0,high=5, size=LINK_COUNT)
#print '1'
t=0
time.sleep(2)
while t< LINK_COUNT:
    ...
    src = nodes[links[t][0]*3-1]
    dst = nodes[links[t][1]*3-1]
    port=np.random.randint(low=1800,high=1899)
    ...
    if socket_exist(back_traffic,src, dst,port) is 'match' or src == dst or net.IPNetwork(src_) == net.IPNetwork(dst_):
	print 'SOCKET_EXIST ERROR %s %s === j:%s' % (dst,port,t)
    else:
	    
	start_time=t+1
	duration=90
	delay=0
	back_traffic[t]=(start_time,src,dst,port,duration,delay)
	background(start_time,src,dst,port,duration,0)
	#time.sleep(0.5) 
	background(start_time,dst,src,port,duration,0)
